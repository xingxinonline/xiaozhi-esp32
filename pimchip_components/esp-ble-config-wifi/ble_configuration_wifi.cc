...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "LanDouBao_XXXX";
    ...
    int32_t event_id, void* event_data)
{
...
#if 0   //挪到获得IP后再重启，避免获取IP较慢的情况下，设备重启了小程序认为配网失败
            //esp_restart();
            xTaskCreate([](void *ctx) {
                // 等待200ms确保HTTP响应完全发送
                vTaskDelay(pdMS_TO_TICKS(200));
    ...
    case WIFI_EVENT_SCAN_DONE: {
        ...
        {
            ESP_LOGI(TAG,"found wifi:%s", ap_list[i].ssid);
            for (j = 0; j < apCount_no_same; j++){
                if ((memcmp(blufi_ap_list[j].ssid, ap_list[i].ssid, sizeof(ap_list[i].ssid)) == 0)){
                    break;
                }
            }

            if (j >= apCount_no_same){
                blufi_ap_list[apCount_no_same].rssi = ap_list[i].rssi;
                memcpy(blufi_ap_list[apCount_no_same].ssid, ap_list[i].ssid, sizeof(ap_list[i].ssid));
                apCount_no_same++;
            }
        }

        if (ble_is_connected == true) {
            esp_blufi_send_wifi_list(apCount_no_same, blufi_ap_list);
        } else {
            ESP_LOGI(TAG,"BLUFI BLE is not connected yet\n");
        }
